<template>
  <div class="anatomy-table-wrapper" :class="{ rtl: isRtl }">
      ...
      <table class="anatomy-table">
        ...
        <tbody>
          <tr v-for="(row, index) in rows" :key="index">
            ...
            <td>
              <BaseInput v-model="row.count" type="number" min="1" size="sm" />
            </td>
            <td>
              <div class="anatomy-image-upload">
                <input
                  :id="'anatomy-img-' + index"
                  type="file"
                  hidden
                ...
                <label :for="'anatomy-img-' + index" class="upload-trigger-sm">
                  ...
                  >
                    <path
                      d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h7M16 5l2 2 4-4"
                    />
                  </svg>
                </label>
              </div>
    ...
    <div class="mt-1-5 flex justify-end">
      <BaseButton :loading="saving" @click="saveAnatomy">{{
        t('Save Table', 'حفظ الجدول')
      }}</BaseButton>
    </div>
  </div>
</template>
...
<style scoped>
  ...
  .rtl .anatomy-table th,
  .rtl .anatomy-table td {
    text-align: right;
  }
  .rtl .suggestions-popover {
    text-align: right;
  }
</style>
