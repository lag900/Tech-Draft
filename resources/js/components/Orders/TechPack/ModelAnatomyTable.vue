          ...
          <tr v-for="(item, idx) in items" :key="idx">
            <td>
              <div class="anatomy-thumbnail">
                <img v-if="item.img || item.image || item.image_path" :src="resolveImagePath(item.img || item.image || item.image_path)" />
                <div v-else class="null-thumb">---</div>
              </div>
            </td>
            <td class="al-l fw-900">{{ item.piece || item.piece_name }}</td>
            <td class="al-l description-cell">{{ item.part || item.part_description || item.part_name }}</td>
