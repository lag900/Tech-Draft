<template>
  <div class="tech-pack-print" v-if="order">
    ...
    <div class="a4-page">
      ...
      <div class="page-content">
        <ProductSpecifications 
          ...
          :styleName="order.title"
          :clientName="order.client?.brand_name || order.client_name || '---'"
          :category="order.category?.name || order.category_name || '---'"
          :season="order.season + ' ' + (order.year || '')"
          :targetMarket="order.target_market || 'Global'"
    ...
    <div class="a4-page">
      ...
      <div class="page-content">
        <ModelAnatomyTable v-if="can('orders.view')" :items="order.anatomy || order.anatomies || []" />

        <div class="row-flex mt-20">
          <div class="col-50">
            <ConstructionSpecs 
              v-if="can('orders.view')"
              :stitchType="order.construction?.stitch_type"
              :seamType="order.construction?.seam_type"
              :spi="'10-12'"
      ...
      <div class="page-content">
        ...
        <div class="row-flex mt-20">
          ...
          <div class="col-50">
            <LabelingPlacement 
              v-if="can('orders.view')"
              :mainLabel="order.packaging?.main_label"
              :sizeLabel="order.packaging?.size_label"
              :careLabel="order.packaging?.care_label"
              :placement="order.packaging?.placement"
            />
            <PackagingShipping 
              v-if="can('orders.view')"
              :packType="order.packaging?.packaging_type"
              :foldingMethod="order.packaging?.folding_method"
              :cartonQty="'Assorted'"
              :barcodeRequired="order.packaging?.barcode_required"
            />
          </div>
        </div>
...
const totalQuantity = computed(() => {
  const breakdown = order.value?.measurements?.breakdown || order.value?.production_details?.measurements?.breakdown;
  if (!breakdown) return 0;
  return breakdown.reduce((acc, s) => acc + (s.quantity || 0), 0);
});

const measurementBreakdown = computed(() => order.value?.measurements?.breakdown || order.value?.production_details?.measurements?.breakdown || []);
const measurementPoints = computed(() => order.value?.measurements?.points || order.value?.production_details?.measurements?.points || []);
const activeMarker = computed(() => order.value?.marker_plan || order.value?.marker_plans?.[0] || null);
...
const bomData = computed(() => {
  ...
  return [
    { name: 'Main Fabric', desc: fab.type || 'Standard', color: o.colors?.[0]?.color_name || 'Specified', unit: 'MTR' },
    { name: 'Construction Thread', desc: trm.thread || 'Polyester 120', color: 'DTM', unit: 'CONE' },
