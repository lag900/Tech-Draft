<template>
  <Layout>
    <div class="roles-view" :class="{ rtl: isRtl }">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">
            {{
              t(
                'Manage system roles and access control levels',
                'إدارة أدوار النظام ومستويات التحكم في الوصول'
              )
            }}
          </p>
        </div>
        <div class="header-actions">
          <BaseButton variant="white" class="mr-2" @click="$router.push('/settings/permissions')">
            <template #icon-left>
              <svg
                width="20"
                height="20"
      ...
      <div v-if="!loading" class="roles-grid">
        <BaseCard
          v-for="role in allRoles"
          :key="role.slug"
          class="role-card"
          :class="{ 'is-disabled': role.status === 'disabled' }"
        >
          <div class="role-header">
            <div
              class="role-icon-box"
              :style="{
                backgroundColor: (role.color || '#64748b') + '15',
            ...
            <div class="role-meta">
              <div class="name-status">
                ...
                <span class="status-badge" :class="role.status">{{
                  t(role.status, role.status === 'active' ? 'نشط' : 'معطل')
                }}</span>
              </div>
          ...
          <p class="role-description">
            {{
              t(role.description, role.description) || t('No description provided.', 'لا يوجد وصف.')
            }}
          </p>
      ...
      <BaseModal
        ...
        :subtitle="
          t('Define security permissions and access level', 'تحديد صلاحيات الأمان ومستوى الوصول')
        "
        size="lg"
        :is-rtl="isRtl"
      ...
      >
        <div class="modal-form">
          <BaseInput
            v-model="form.name"
            :label="t('Role Name', 'اسم الدور')"
            :placeholder="t('Ex: Quality Inspector', 'مثال: مراقب جودة')"
            required
            class="mb-2"
          />
          <BaseTextarea
            ...
            :placeholder="
              t('Describe the responsibilities of this role...', 'وصف مسؤوليات هذا الدور...')
            "
            rows="3"
          />
        </div>
        ...
        <template #footer>
          <BaseButton variant="white" @click="showCreateModal = false">{{
            t('Cancel', 'إلغاء')
          }}</BaseButton>
...
<script setup>
  ...
  const allRoles = computed(() => {
    return customRoles.value.map((role) => ({
      ...role,
      isSystem: ['superadmin', 'admin', 'manager', 'designer', 'dataentry', 'client'].includes(
        role.slug
      ),
  ...
  const toggleRoleStatus = async (role) => {
    ...
    try {
      await axios.patch(
        `/api/roles/${role.id}/toggle-status`,
        {},
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('auth_token')}` },
        }
