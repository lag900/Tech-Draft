    ...
    <div class="settings-view" :class="{ rtl: isRtl }">
      <div class="page-header">
        ...
        <p class="page-subtitle">
          {{
            t(
              'Manage platform configuration, roles, and security policies',
              'إدارة تكوين المنصة، الأدوار، وسياسات الأمان'
            )
          }}
        </p>
      </div>
      ...
      <div class="settings-grid">
        ...
        <main class="settings-content">
          <BaseCard
            ...
            :title="t('Platform Configuration', 'تكوين المنصة')"
          >
            <div class="form-grid">
          ...
          <div v-if="activeTab === 'rbac'" class="rbac-settings">
            <div class="rbac-cards">
              <BaseCard
                :title="t('Security Access Matrix', 'مصفوفة الوصول الأمني')"
                subtitle="Map modules and actions to specific roles"
              >
                <template #actions>
                  <BaseButton
                    variant="ghost"
                    ...
                    >{{ t('Open Matrix', 'فتح المصفوفة') }}</BaseButton
                  >
                </template>
                <p class="settings-desc">
                  {{
                    t(
                      ...
                      'تكوين أذونات دقيقة لكل دور عبر جميع وحدات النظام.'
                    )
                  }}
                </p>
              </BaseCard>
              ...
              <BaseCard
                :title="t('Identity Registry (Roles)', 'سجل الهويات (الأدوار)')"
                subtitle="Manage role descriptions and role-user mapping"
              >
                <template #actions>
                  <BaseButton variant="ghost" size="sm" @click="$router.push('/settings/roles')">{{
                ...
                <p class="settings-desc">
                  {{
                    t(
                      'Edit role details, descriptions, and view how many users are assigned to each identity.',
                      'تحرير تفاصيل الأدوار، الأوصاف، وعرض عدد المستخدمين المخصصين لكل هوية.'
                    )
                  }}
                </p>
              </BaseCard>
          ...
          <div v-if="activeTab === 'landing_page'" class="landing-cms-settings">
            <BaseCard
              :title="t('Landing Page CMS', 'إدارة الصفحة الرئيسية')"
              subtitle="Manage the content of the welcome page"
            >
              <template #actions>
                <BaseButton
                  variant="ghost"
                  size="sm"
                  @click="$router.push('/settings/landing-page')"
                  >{{ t('Open CMS', 'فتح إدارة الصفحة') }}</BaseButton
                >
              </template>
              <p class="settings-desc">
                {{
                  t(
                    'Edit hero section, features, steps, and call-to-action details dynamically.',
                    'قم بتعديل القسم الترحيبي، المميزات، الخطوات، وتفاصيل الدعوة لاتخاذ إجراء بأسلوب تفاعلي.'
                  )
                }}
              </p>
            </BaseCard>
...
<style scoped>
  ...
  .rtl .nav-btn {
    flex-direction: row;
  } /* Standard flex row is fine, but gap needs to keep consistency */
  .rtl .footer-actions {
