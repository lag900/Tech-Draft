  ...
  <Layout>
    <div class="product-management-view" :class="{ rtl: isRtl }">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">
            {{
              t(
                'Manage finished products, inventory and pricing',
                'إدارة المنتجات النهائية، المخزون والتسعير'
              )
            }}
          </p>
        </div>
        ...
        <div class="filter-bar">
          <div class="search-box">
            ...
            <input
              ...
              type="text"
              :placeholder="t('Search SKU, name...', 'بحث بالباركود، الاسم...')"
            />
          </div>
          ...
          <div class="product-info">
            <div class="p-header">
              ...
              <span class="status-badge" :class="product.status">{{
                t(product.status, product.status === 'active' ? 'نشط' : 'معطل')
              }}</span>
            </div>
            <h3 class="p-name">{{ product.name }}</h3>
            <p v-if="product.category" class="p-category">{{ product.category.name }}</p>
            <div class="p-footer">
      ...
      <BaseCard v-else class="empty-state-card">
        <div class="empty-v">
          ...
          <p>
            {{
              t(
                "Click 'Create Product' to add your first item to the catalog.",
                "اضغط على 'إنشاء منتج' لإضافة أول قطعة للكتالوج."
              )
            }}
          </p>
          <BaseButton
            ...
            >{{ t('Create Product', 'إنشاء منتج') }}</BaseButton
          >
        </div>
      </BaseCard>
      ...
      <BaseModal
        :show="showModal"
        :title="
          editingProduct ? t('Edit Product', 'تعديل منتج') : t('Create Product', 'إنشاء منتج')
        "
      ...
      >
        ...
        <template #footer>
          ...
          <BaseButton :loading="saving" @click="saveProduct">{{ t('Save', 'حفظ') }}</BaseButton>
        </template>
      </BaseModal>
