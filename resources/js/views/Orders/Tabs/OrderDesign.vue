<template>
  <div class="tab-pane animate-fade-in">
    <div class="design-visualization-grid">
      ...
      <div class="visual-card">
        ...
        <div class="visual-canvas" @click="zoomImage(order.design_front_image)">
          ...
          <div v-else class="empty-canvas">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16M14 14l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
      ...
      <div class="visual-card technical">
        ...
        <div class="visual-canvas" @click="zoomImage(order.technical_sketch)">
          ...
          <div v-else class="empty-canvas">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
      ...
      <div class="visual-card detail">
        ...
        <div class="visual-canvas" @click="zoomImage(order.detail_zoom)">
          ...
          <div v-else class="empty-canvas">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35M11 8v6M8 11h6"/></svg>
    ...
    <BaseCard v-if="order.images?.length > 0" :title="t('Reference Gallery', 'معرض الصور المرجعية')" class="mt-1">
       <div class="gallery-grid-compact">
            <div v-for="img in order.images" :key="img.id" class="gallery-img-box-sm" @click="zoomImage(img.file_path)">
...
.gallery-img-box-sm:hover img { transform: scale(1.05); }

.colors-flex-compact { display: flex; flex-wrap: wrap; gap: 0.75rem; }
.color-item-sm { 
    display: flex; 
    align-items: center; 
    gap: 0.75rem; 
...
.color-circle-sm { width: 36px; height: 36px; border-radius: 8px; border: 1px solid rgba(0,0,0,0.1); flex-shrink: 0; }
.color-meta-sm .name { font-size: 0.8125rem; font-weight: 800; color: #1e293b; }
.color-meta-sm .code { font-size: 0.65rem; color: #94a3b8; font-family: monospace; }
