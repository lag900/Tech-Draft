<template>
  <div class="tab-pane animate-fade-in">
    <div class="tech-spec-grid">
      ...
      <BaseCard :title="t('Construction Specifications', 'مواصفات الإنشاء والتجميع')" class="spec-card">
        <div class="info-grid-2">
           ...
           <div class="info-field">
             ...
             <span class="value">{{ order.production_details?.seam_type || form.seam_type || '---' }}</span>
           </div>
           <div class="info-field">
             <span class="label">{{ t('SPI Target (Stitches Per Inch)', 'عدد الغرز في البوصة (SPI)') }}</span>
             <span class="value">{{ order.production_details?.spi || '10-12 SPI' }}</span>
           </div>
           <div class="info-field">
             <span class="label">{{ t('Reinforcement', 'التقوية / التثبيت') }}</span>
             <span class="value">{{ order.production_details?.reinforcement || 'Back-tack all corners' }}</span>
           </div>
        </div>
      </BaseCard>
    ...
    <BaseCard :title="t('Model Anatomy (Parts List)', 'تشريح الموديل (قائمة الأجزاء)')" class="mt-1">
          ...
          <div v-for="(part, idx) in anatomies" :key="idx" class="anatomy-card-sm">
              <div class="part-image-box">
                ...
                <div v-else class="part-img-placeholder">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20 21v-2a4 4 0 00-3-3.87m-7 3.87v-2a4 4 0 013-3.87M12 3.13a4 4 0 010 7.75"/></svg>
                </div>
              </div>
              <div class="anatomy-details-sm">
                ...
                <div class="p-meta">
                   ...
                   <span class="p-note">{{ part.part_description || t('Body Item', 'قطعة أساسية') }}</span>
                </div>
              </div>
          </div>
...
.p-name { font-size: 0.75rem; font-weight: 700; color: #1e293b; }
.p-qty { font-size: 0.7rem; font-weight: 800; color: #0ea5e9; background: #e0f2fe; padding: 2px 6px; border-radius: 4px; }
.mt-1 { margin-top: 1rem; }
.empty-state-sm { font-size: 0.875rem; color: #94a3b8; font-style: italic; }
</style>
