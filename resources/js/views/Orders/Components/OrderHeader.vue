<template>
  <BaseCard class="order-header-card no-print mb-2">
    <div class="header-flex">
      <div class="header-img">
        <img
          :src="order.product_image_url || '/images/product-placeholder.png'"
          alt="product"
      ...
      <div class="header-info">
        ...
        <div class="header-meta-grid">
          ...
          <div class="h-meta">
            ...
            <span class="h-val"
              >{{ t(order.season, seasonsAr[order.season] || order.season) }} {{ order.year }}</span
            >
          </div>
        </div>
        <div class="header-status-row">
          <span class="status-pill-large" :class="order.status">
            {{
              t(
                statusMap[order.status]?.en || order.status,
                statusMap[order.status]?.ar || order.status
              )
            }}
          </span>
          <span class="h-dates">
...
<script setup>
  ...
  const formatDate = (date) =>
    new Date(date).toLocaleDateString(props.isRtl ? 'ar-EG' : 'en-US', {
      day: 'numeric',
      month: 'short',
      year: 'numeric',
    });
...
<style scoped>
  ...
  .status-pill-large.technical_ready {
    background: #f0f9ff;
    color: #0ea5e9;
    border-color: #bae6fd;
  }
  .status-pill-large.in_review {
    background: #eff6ff;
    color: #1e40af;
    border-color: #dbeafe;
  }
  .status-pill-large.sampling {
    background: #f5f3ff;
    color: #6d28d9;
    border-color: #ede9fe;
  }
