        ...
        <div class="header-left">
           ...
           <div class="title-with-avatar">
              ...
              <div>
                 ...
                 <p class="page-subtitle">{{ client.client?.company_name }} • {{ client.client?.country }}</p>
              </div>
           </div>
        </div>
        <div class="header-actions">
           <BaseButton variant="white" @click="$router.push('/messages')">{{ t('Send Message', 'إرسال رسالة') }}</BaseButton>
        </div>
      </div>
            ...
            <BaseCard class="info-section" :title="t('Recent Activity', 'النشاط الأخير')">
               <div class="table-responsive">
                  <table class="modern-table">
                     ...
                     <tbody>
                        <tr v-for="order in client.created_orders" :key="order.id" class="clickable-row" @click="$router.push('/orders/'+order.id)">
                           <td><span class="code">#{{ order.order_code }}</span></td>
                           <td><span class="title">{{ order.title }}</span></td>
                           <td>
                        ...
                        <tr v-if="!client.created_orders?.length">
                           <td colspan="4" class="empty-text">{{ t('No recent orders.', 'لا يوجد طلبات حديثة.') }}</td>
                        </tr>
                     </tbody>
                  </table>
               </div>
         ...
         <aside class="detail-sidebar">
            <BaseCard class="insight-card" :title="t('Business Insights', 'رؤى الأعمال')">
               <div class="insight-item">
                  ...
                  <span class="i-val">~{{ Math.round(client.total_units / (client.total_orders || 1)) || 0 }}</span>
               </div>
               <div class="insight-item">
            ...
            <BaseCard class="help-box mt-2">
               ...
               <textarea class="internal-notes-area" placeholder="Add note about this client..."></textarea>
               <BaseButton variant="white" block size="sm" class="mt-1">{{ t('Save Note', 'حفظ الملاحظة') }}</BaseButton>
            </BaseCard>
         </aside>
      </div>
    </div>
...
const formatDate = (date) => {
   if (!date) return '---';
   return new Date(date).toLocaleDateString(isRtl.value ? 'ar-EG' : 'en-US', { day: 'numeric', month: 'short', year: 'numeric' });
};
</script>

<style scoped>
.client-detail-view { max-width: 1200px; margin: 0 auto; padding-bottom: 5rem; }

.page-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 2.5rem; }
.breadcrumb-mini { display: flex; align-items: center; gap: 0.5rem; font-size: 0.75rem; font-weight: 700; color: #94a3b8; margin-bottom: 1rem; }
.breadcrumb-mini a { color: inherit; text-decoration: none; }
.breadcrumb-mini .sep { opacity: 0.5; }
.breadcrumb-mini .active { color: #0ea5e9; }

.title-with-avatar { display: flex; align-items: center; gap: 1.5rem; }
.client-large-avatar { width: 72px; height: 72px; border-radius: 20px; background: #1a3a5f; color: white; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 2rem; }
.page-title { font-size: 2.25rem; font-weight: 800; color: #1a3a5f; margin: 0; letter-spacing: -0.02em; }
.page-subtitle { color: #64748b; font-size: 1rem; margin-top: 4px; font-weight: 600; }

.stats-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1.5rem; }
.stat-box { padding: 1.5rem !important; display: flex; flex-direction: column; gap: 4px; }
...
.detail-grid { display: grid; grid-template-columns: 1fr 340px; gap: 2rem; align-items: flex-start; }
@media (max-width: 1024px) { .detail-grid { grid-template-columns: 1fr; } }

.profile-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; padding: 1rem 0; }
...
.modern-table th { text-align: left; padding: 1rem; border-bottom: 2px solid #f1f5f9; font-size: 0.7rem; font-weight: 800; color: #94a3b8; text-transform: uppercase; }
.modern-table td { padding: 1rem; border-bottom: 1px solid #f8fafc; font-size: 0.875rem; font-weight: 600; }

.code { color: #0ea5e9; font-family: monospace; font-size: 0.9375rem; font-weight: 800; }
.status-pill { padding: 4px 10px; border-radius: 8px; font-size: 0.7rem; font-weight: 800; text-transform: uppercase; }
.status-pill.sampling, .status-pill.production { background: #fef2f2; color: #ef4444; }
.status-pill.completed { background: #f0fdf4; color: #10b981; }

.insight-card { padding: 1.25rem !important; }
.insight-item { padding: 1rem 0; border-bottom: 1px solid #f1f5f9; display: flex; flex-direction: column; gap: 4px; }
.insight-item:last-child { border: none; }
.i-label { font-size: 0.75rem; color: #64748b; font-weight: 600; }
.i-val { font-size: 1rem; font-weight: 800; color: #1a3a5f; }

.help-box h4 { font-weight: 800; font-size: 0.9375rem; margin: 0 0 0.5rem; color: #1a3a5f; }
.help-box p { font-size: 0.75rem; color: #94a3b8; margin-bottom: 1rem; }
.internal-notes-area { width: 100%; height: 100px; padding: 0.75rem; border-radius: 12px; border: 1px solid #e2e8f0; font-size: 0.875rem; resize: none; }
...
.rtl .page-header, .rtl .modern-table th { text-align: right; }
.rtl .breadcrumb-mini { direction: rtl; }
</style>
