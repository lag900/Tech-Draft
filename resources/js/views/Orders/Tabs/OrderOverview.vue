      ...
      <BaseCard :title="t('Product Specifications', 'مواصفات المنتج')" class="spec-card">
        <div class="info-grid-3">
          ...
          <div class="info-field">
            ...
            <span class="value">{{ t(order.season, seasonsAr[order.season]) }} {{ order.year }}</span>
          </div>
          <div class="info-field">
          ...
          <div class="info-field">
            ...
            <span class="value highlight">{{ order.production_details?.quantity || order.quantity || '---' }}</span>
          </div>
        </div>
      </BaseCard>
      ...
      <BaseCard :title="t('Lifecycle & Status', 'حالة الدورة')" class="spec-card">
        <div class="info-grid-2">
          ...
          <div class="info-field">
            ...
            <span class="value small">{{ formatDate(order.updated_at || order.last_update_date) }}</span>
          </div>
          <div class="info-field">
            <span class="label">{{ t('Version', 'الإصدار') }}</span>
            <span class="value">v{{ order.version_number || 1 }}.{{ order.revision_number || 0 }}</span>
          </div>
        </div>
      </BaseCard>
    </div>

    <BaseCard v-if="order.notes" :title="t('Internal Factory Notes', 'ملاحظات المصنع الداخلية')" class="mt-1">
...
.status-badge.production { background: #dbeafe; color: #1e40af; }
