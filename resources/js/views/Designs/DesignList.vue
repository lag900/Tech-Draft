<template>
  <Layout>
    <div class="designs-view" :class="{ rtl: isRtl }">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">
            {{
              t(
                'Manage product designs and technical specifications',
                'إدارة تصاميم المنتجات والمواصفات الفنية'
              )
            }}
          </p>
        </div>
        <router-link v-if="can('designs.create')" to="/designs/create">
          <BaseButton>
            <template #icon-left>
              <svg
                width="18"
      ...
      <div v-if="loading" class="p-4 text-center">{{ t('Loading...', 'جاري التحميل...') }}</div>
      <div v-else-if="!designs?.length" class="p-4 text-center">
      ...
      <div v-else class="designs-grid">
        ...
        >
          ...
          <div class="design-info">
            ...
            <p class="design-meta">
              {{ t('Category', 'التصنيف') }}: {{ design.category ? design.category.name : 'N/A' }}
            </p>
            <div class="design-footer">
              <span class="design-date">{{ t('By', 'بواسطة') }}: {{ design.creator?.name }}</span>
              <router-link :to="`/designs/${design.id}`" class="view-link">{{
                t('Details', 'التفاصيل')
              }}</router-link>
            </div>
          </div>
