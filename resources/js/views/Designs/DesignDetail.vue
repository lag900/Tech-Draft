    ...
    <div class="design-detail-view dashboard-mode" :class="{ rtl: isRtl }">
      <div class="page-header">
        <div>
          ...
          <p v-if="design" class="page-subtitle">
            {{ design.name }} - {{ design.product_code || '---' }}
          </p>
        </div>
        <div class="header-actions">
          <BaseButton variant="white" @click="$router.push('/designs')">{{
            t('Back to History', 'العودة للتصاميم')
          }}</BaseButton>
          <BaseButton v-if="can('designs.edit')">{{
      ...
      <div v-else class="dashboard-grid">
        ...
        <div class="left-col">
          <BaseCard no-padding class="visual-card">
            ...
            <div class="visual-info p-1-5">
              ...
              <p class="info-desc">
                {{ design.description || t('No description provided.', 'لا يوجد وصف.') }}
              </p>
            </div>
          </BaseCard>
        ...
        <div class="right-col">
          <BaseCard class="orders-card" :title="t('Production Orders', 'طلبات الإنتاج')">
            <div class="mini-order-list">
              ...
              >
                ...
                <div class="mo-meta">{{ formatDate(order.created_at) }} • {{ order.title }}</div>
              </div>
          ...
          <BaseCard class="files-card mt-1-5" :title="t('Attachments', 'المرفقات')">
            ...
            <BaseButton block variant="white" class="mt-1">{{
              t('Upload New File', 'رفع ملف جديد')
            }}</BaseButton>
          </BaseCard>
        </div>
      </div>
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [designRes, logRes] = await Promise.all([
        ...
        axios.get(
          `/api/activity-logs?subject_type=App\\Models\\Design&subject_id=${route.params.id}`,
          { headers }
        ),
      ]);
