        ...
        <table class="tech-table">
          ...
          <tbody>
            <tr v-for="point in points" :key="point.id">
              <td class="sticky-col font-bold">{{ point.point_of_measure }}</td>
              <td v-for="size in availableSizes" :key="size" class="m-val text-center">
                <!-- logic: if it matches base size show value, else show value + grading -->
                {{ getValForSize(point, size) }}
              </td>
              <td class="text-muted text-center">{{ point.tolerance || '±1.0' }}</td>
              <td class="text-xs text-slate-500">{{ point.notes || '---' }}</td>
            </tr>
            <tr v-if="points.length === 0">
              <td :colspan="availableSizes.length + 3" class="text-muted py-8 text-center italic">
                {{
                  t(
                    'No measurements point defined for this style.',
                    'لم يتم تعريف نقاط قياس لهذا الموديل.'
                  )
                }}
              </td>
            </tr>
