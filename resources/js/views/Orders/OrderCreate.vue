  ...
  <Layout>
    <div class="order-create-view" :class="{ rtl: isRtl }">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">
            {{
              t(
                'Submit your technical specifications for production',
                'أرسل مواصفاتك الفنية لبدء عملية التصنيع'
              )
            }}
          </p>
        </div>
      ...
      <form class="order-form" @submit.prevent="submitOrder">
        ...
        <BaseCard
          class="form-section"
          :title="t('Design Information', 'معلومات التصميم')"
          :subtitle="t('Product identity and style', 'هوية المنتج والستايل')"
        >
          <div class="form-grid-top">
              ...
              <SearchableSelect
                ...
                :no-results-message="
                  t(
                    ...
                    'لا توجد تصنيفات متاحة. يرجى الاتصال بالمسؤول.'
                  )
                "
              ...
              <SearchableSelect
                ...
                :no-results-message="
                  t(
                    ...
                    'لم يتم العثور على أنواع قطع لهذا التصنيف.'
                  )
                "
            ...
            <div class="form-grid-2 mt-2">
              <SearchableSelect
                ...
                required
                :no-results-message="
                  t('No fit types available for this item.', 'لا توجد قصات متاحة لهذه القطعة.')
                "
                :error="errors.fit_id"
              />
              ...
              <div class="field-item">
                ...
                <div class="checkbox-group">
                  <label v-for="sea in seasonOptions" :key="sea.value" class="check-label">
                    <input v-model="form.seasons" type="checkbox" :value="sea.value" />
                    {{ sea.label }}
                  </label>
                </div>
        ...
        <BaseCard
          class="form-section fabric-section"
          :title="t('Fabric & Material', 'الأقمشة والمواد')"
          :subtitle="
            t('Define the primary material for this design', 'حدد المادة الأساسية لهذا التصميم')
          "
        >
          <div class="form-grid-3">
            <SearchableSelect
              ...
              required
              :no-results-message="
                t('No fabrics available in stock.', 'لا توجد أقمشة متاحة في المخزن.')
              "
            />
            <div class="field-item">
              <BaseInput
                ...
                type="number"
                :label="t('Fabric Weight (gsm)', 'وزن القماش (جرام)')"
                placeholder="320"
                required
              />
              <p class="field-help">
                {{ t('Grams per square meter (GSM)', 'متر لكل جرام / جرام لكل متر مربع') }}
              </p>
            </div>
            <BaseInput
              v-model="form.texture"
              :label="t('Fabric Texture', 'ملمس القماش')"
              :placeholder="t('Ex: Soft, Ribbed...', 'مثال: ناعم، مضلع...')"
            />
          </div>
          <div class="form-grid-3 mt-2">
            <BaseInput
              v-model="form.fabric_composition"
              :label="t('Composition', 'تكوين القماش')"
              placeholder="80% Cotton / 20% Poly"
            />
            <BaseInput
              v-model="form.yarn_type"
              :label="t('Yarn Type', 'نوع الخيط')"
              placeholder="Ring Spun"
            />
            <BaseInput
              v-model="form.fabric_structure"
              :label="t('Fabric Structure', 'هيكل القماش')"
              placeholder="Single Jersey, Pique..."
            />
          </div>
          <div v-if="user.role === 'admin'" class="form-grid-2 mt-2 border-t pt-2">
            <BaseInput
              v-model="form.fabric_code"
              :label="t('Internal Fabric Code', 'كود القماش الداخلي')"
            />
            <BaseInput
              v-model="form.fabric_supplier"
              :label="t('Fabric Supplier', 'مورد القماش')"
            />
          </div>
        </BaseCard>
        ...
        <BaseCard
          class="form-section"
          :title="t('Sizes & Quantity', 'المقاسات والكمية')"
          :subtitle="
            t('Define size distribution and total volume', 'حدد توزيع المقاسات والكمية الإجمالية')
          "
        >
          <div class="size-management-container">
            ...
            <div class="size-breakdown-table mb-1">
              <div v-for="(item, idx) in form.size_breakdown" :key="idx" class="size-row">
                ...
                />
                <button
                  v-if="form.size_breakdown.length > 1"
                  type="button"
                  class="remove-btn"
                  @click="form.size_breakdown.splice(idx, 1)"
                >
                  <svg
                    width="12"
                    height="12"
            ...
            <div class="total-qty-footer">
              <div class="total-label-box">
                <span class="total-label">{{
                  t('Total Quantity (Auto)', 'إجمالي الكمية (تلقائي)')
                }}</span>
              ...
              <TransitionGroup name="list">
                <div v-for="(color, index) in form.colors" :key="index" class="color-card">
                  <div class="color-swatch-row">
                    <input v-model="color.hex" type="color" class="mini-color-picker" />
                  ...
                  <input
                    ...
                    class="mini-input mb-05"
                  />
                  <input
                    v-model="color.code"
                    type="text"
                    :placeholder="t('Pantone', 'بانتون')"
                    class="mini-input"
                  />
                  <button
        ...
        <BaseCard
          class="form-section"
          :title="t('Technical Measurements', 'القياسات الفنية')"
          :subtitle="t('Specify dimensions in cm', 'يرجى تحديد الأبعاد بالسنتيمتر')"
        >
          <div v-if="templateSuggestions.length" class="suggestions-area mb-1-5">
            <p class="suggestions-title">
              {{
                t('Suggested Templates (From your past orders)', 'قوالب مقترحة (من طلباتك السابقة)')
              }}
            </p>
        ...
        <BaseCard class="form-section" :title="t('Branding & Notes', 'البراندنج والملاحظات')">
          <div class="specs-group mb-2">
            ...
            <div class="form-grid-2">
              <BaseInput
                ...
                :label="t('Main Label Type', 'نوع الليبل الرئيسي')"
                placeholder="Woven, Silicon..."
              />
              <BaseInput
                v-model="form.size_label_type"
                :label="t('Size Label Type', 'نوع ليبل المقاس')"
              />
            </div>
            <div class="form-grid-2 mt-1">
              <BaseInput
                v-model="form.care_label_type"
                :label="t('Care Label Instructions', 'تعليمات الغسيل')"
                placeholder="Wash 30°C, No Bleach..."
              />
              <div class="field-item">
                <label class="input-label">{{ t('Hang Tag Required', 'تاق خارجي مطلوب؟') }}</label>
                <div class="checkbox-group">
                  <input v-model="form.hangtag_required" type="checkbox" />
                  <span>{{ t('Yes', 'نعم') }}</span>
                </div>
              </div>
            </div>
          ...
          <div class="specs-group">
            ...
            <textarea
              ...
              :placeholder="
                t(
                  'Add instructions for logo placement, embroidery, special packaging...',
                  'أضف تعليمات لمكان اللوجو، التطريز، التغليف الخاص...'
                )
              "
            ></textarea>
          </div>
        ...
        <BaseCard
          ...
          class="form-section admin-specs-card"
          :title="t('Factory Technical Specs', 'المواصفات الفنية للمصنع')"
        >
          <div class="specs-group mb-2">
            ...
            <div class="form-grid-2 mt-1">
              ...
              <BaseInput
                ...
                :label="t('Stitching Type', 'نوع الخياطة')"
              />
            </div>
          </div>
          ...
          <div class="specs-group">
            ...
            <div class="form-grid-3">
              ...
              <div class="field-item">
                ...
                <div class="checkbox-group">
                  <input v-model="form.barcode_required" type="checkbox" />
                  <span>{{ t('Yes', 'نعم') }}</span>
                </div>
              </div>
            </div>
        ...
        <BaseCard
          class="form-section"
          :title="t('Upload Design Files', 'رفع ملفات التصميم')"
          :subtitle="
            t(
              'Upload sketches, technical drawings, or reference photos',
              'ارفع سكتشات، رسومات فنية، أو صور مرجعية'
            )
          "
        >
            ...
            <div class="asset-box">
              ...
              <div class="photo-u" @click="$refs.frontInput.click()">
                <input
                  ...
                  type="file"
                  hidden
                  @change="(e) => handleAssetUpload(e, 'design_front_image')"
                />
                <img
                  v-if="previews_special.design_front_image"
                  :src="previews_special.design_front_image"
                />
            ...
            <div class="asset-box">
              ...
              <div class="photo-u" @click="$refs.backInput.click()">
                <input
                  ...
                  type="file"
                  hidden
                  @change="(e) => handleAssetUpload(e, 'design_back_image')"
                />
                <img
                  v-if="previews_special.design_back_image"
                  :src="previews_special.design_back_image"
                />
                <div v-else class="u-placeholder">+</div>
              </div>
            </div>
            <div class="asset-box">
              ...
              <div class="photo-u" @click="$refs.sketchInput.click()">
                <input
                  ...
                  type="file"
                  hidden
                  @change="(e) => handleAssetUpload(e, 'technical_sketch')"
                />
                <img
                  v-if="previews_special.technical_sketch"
                  :src="previews_special.technical_sketch"
                />
                <div v-else class="u-placeholder">+</div>
              </div>
            </div>
          ...
          >
            ...
            <div class="upload-v">
              ...
              <p class="upload-title">
                <strong>{{ t('Click to upload', 'اضغط للرفع') }}</strong>
                {{ t('reference images', 'أو ارفع صور مرجعية') }}
              </p>
              <p class="upload-subtitle">
                {{ t('PNG, JPG or PDF up to 10MB', 'PNG، JPG أو PDF بحد أقصى ١٠ ميجابايت') }}
              </p>
            </div>
          </div>

          <div v-if="previews.length" class="image-gallery mt-2">
            <TransitionGroup name="list">
        ...
        <BaseCard
          class="form-section"
          :title="t('Additional Notes', 'ملاحظات إضافية')"
          :subtitle="t('Extra instructions', 'تعليمات إضافية للمصنع')"
        >
          <BaseTextarea
            v-model="form.notes"
            :placeholder="
              t(
                'Describe labels, packaging, or special stitching...',
                'صف الملصقات، التغليف، أو خياطة خاصة...'
              )
            "
            rows="5"
          />
...
<script setup>
    ...
    try {
      ...
      if (route.query.reorder_id) {
        ...
        if (reorderRes.data) {
          ...
          showAlert(
            t('Template applied from past order.', 'تم تطبيق القالب من الطلب السابق.'),
            '',
            'success'
          );
        }
  ...
  const fabricOptions = computed(() =>
    fabricList.value.map((f) => ({
      value: f.id,
      label: `${f.name} - ${f.material_type || 'Unknown'}`,
    }))
  );
  ...
  const itemTypeOptions = computed(() =>
    itemTypes.value.map((i) => ({ value: i.id, label: i.name }))
  );
  const fitOptions = computed(() => fits.value.map((f) => ({ value: f.id, label: f.name })));
  const clientOptions = computed(() =>
    clients.value.map((c) => ({ value: c.id, label: c.brand_name || c.name }))
  );
    ...
    async (newVal) => {
      ...
      if (newVal) {
        const [fitRes, tempRes] = await Promise.all([
          ...
          axios
            .get(`/api/measurement-templates/${newVal}`, { headers })
            .catch(() => ({ data: null })),
        ]);
  ...
  const applyTemplate = (order) => {
    ...
    if (order.season) {
      ...
      form.seasons = seasonOptions.value
        .filter((opt) => seasonsArr.includes(opt.value))
        .map((opt) => opt.value);
    }
  ...
  const handleAddSize = (newSize) => {
    ...
    if (
      form.size_breakdown.length === 1 &&
      !form.size_breakdown[0].size &&
      !form.size_breakdown[0].qty
    ) {
  ...
  const submitOrder = async () => {
    if (!form.title || !form.seasons.length) {
      showAlert(
        t(
          'Please fill all required fields, including Season.',
          'الرجاء ملء جميع الحقول المطلوبة، بما في ذلك الموسم.'
        ),
        '',
        'error'
      );
    ...
    if (form.quantity <= 0) {
      showAlert(
        t(
          'Please provide quantities in the Size Breakdown table.',
          'يرجى تقديم الكميات في جدول توزيع المقاسات.'
        ),
        '',
        'error'
      );
    ...
    formData.append(
      ...
      JSON.stringify({
        ...
        sizes_text: form.size_breakdown
          .filter((i) => i.size)
          .map((i) => i.size)
          .join(', '),
    ...
    const sizesText = form.size_breakdown
      .filter((i) => i.size)
      .map((i) => i.size)
      .join(', ');
    ...
    try {
      ...
      if (e.response?.status === 422) {
        ...
        if (backendErrors) {
          ...
          showAlert(
            t('Please correct the highlighted errors.', 'الرجاء تصحيح الأخطاء الموضحة.'),
            '',
            'error'
          );
        }
        ...
        showAlert(
          t(
            'Error submitting order: ' + (e.response?.data?.message || e.message),
            'فشل إرسال الطلب: ' + (e.response?.data?.message || e.message)
          ),
          '',
          'error'
        );
