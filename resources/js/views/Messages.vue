      ...
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">
            {{
              t(
                'Manage your production chats with the factory team',
                'أدر محادثات الإنتاج مع فريق المصنع'
              )
            }}
          </p>
        </div>
            ...
            >
              ...
              <div class="order-chat-info">
                <div class="top-row">
                  ...
                  <span v-if="order.last_message" class="chat-time">{{
                    formatTimeShort(order.last_message.created_at)
                  }}</span>
                </div>
                <div class="order-title-mini">{{ order.title || order.category?.name }}</div>
                <div v-if="order.last_message" class="last-msg-preview">
            ...
            <div class="chat-header-main" @click="$router.push('/orders/' + selectedOrderId)">
              <div class="header-info">
                ...
                <span class="header-title">{{
                  selectedOrder?.title || selectedOrder?.category?.name
                }}</span>
              </div>
              <BaseButton variant="ghost" size="sm">{{
                t('View Tech Pack', 'عرض الملف الفني')
              }}</BaseButton>
            </div>

            <div ref="pool" class="message-pool">
              <div
                v-for="msg in messages"
                :key="msg.id"
                :class="['msg-bubble', msg.sender_id === user.id ? 'mine' : 'theirs']"
              >
                <div class="bubble-content">
                  <div v-if="msg.sender_id !== user.id" class="sender-name">
                    {{ msg.sender?.name || 'Factory' }}
                  </div>
              ...
              <div v-if="messages.length === 0" class="empty-chat-v">
                <p>
                  {{
                    t(
                      'Send a message to start the conversation for this order.',
                      'أرسل رسالة لبدء المحادثة لهذا الطلب.'
                    )
                  }}
                </p>
              </div>
...
<script setup>
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const receiverId =
        selectedOrder.value.created_by === user.value.id ? 1 : selectedOrder.value.created_by;
      await axios.post(
        '/api/messages',
        {
  ...
  const scrollToBottom = () => {
    nextTick(() => {
      if (pool.value) pool.value.scrollTop = pool.value.scrollHeight;
    });
  };

  const formatTime = (date) =>
    new Date(date).toLocaleString(isRtl.value ? 'ar-EG' : 'en-US', {
      hour: '2-digit',
      minute: '2-digit',
    });
...
<style scoped>
  ...
  .order-chat-item.active {
    background: #f0f9ff;
    border-left: 4px solid #0ea5e9;
  }
  .rtl .order-chat-item.active {
    border-left: none;
  ...
  .chat-footer-main textarea:focus {
    border-color: #0ea5e9;
    outline: none;
  }
  ...
  .rtl .msg-bubble.mine {
    align-self: flex-start;
  } /* Overridden by LTR container logic sometimes */
  .rtl .msg-bubble.theirs {
    align-self: flex-end;
  }
</style>
