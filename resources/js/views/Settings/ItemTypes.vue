    ...
    <div class="item-types-view" :class="{ 'rtl': isRtl }">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">{{ t('Manage item types linked to categories', 'إدارة أنواع القطع المرتبطة بالتصنيفات') }}</p>
        </div>
          ...
          <table class="modern-table">
            ...
            <tbody>
              <tr v-for="item in itemTypes" :key="item.id">
                ...
                <td><span class="category-text">{{ item.category?.name || '---' }}</span></td>
                <td>
                  <span class="status-badge" :class="item.status">
                    {{ t(item.status === 'active' ? 'Active' : 'Inactive', item.status === 'active' ? 'نشط' : 'غير نشط') }}
                  </span>
                </td>
                <td class="text-right">
                  <div class="action-btns">
                    <button v-if="can('item_types.edit')" class="action-icon-btn edit" @click="openEditModal(item)" :title="t('Edit', 'تعديل')">
                    ...
                    <button v-if="can('item_types.delete')" class="action-icon-btn del" @click="deleteItem(item.id)" :title="t('Delete', 'حذف')">
              ...
              <tr v-if="itemTypes.length === 0">
                <td colspan="4" class="empty-state">{{ t('No item types found.', 'لم يتم العثور على أنواع قطع.') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <BaseModal 
        :show="showModal" 
        :title="editingItem ? t('Edit Item Type', 'تعديل نوع قطعة') : t('Add New Item Type', 'إضافة نوع قطعة جديد')"
        :is-rtl="isRtl"
        @close="closeModal"
      >
        ...
        <template #footer>
          ...
          <BaseButton @click="saveItem" :loading="saving">{{ t('Save Changes', 'حفظ التغييرات') }}</BaseButton>
        </template>
      </BaseModal>
    </div>
  </Layout>
...
.item-name { font-weight: 700; color: #1a3a5f; }
.category-text { color: #64748b; font-weight: 600; }
