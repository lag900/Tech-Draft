  ...
  <Layout>
    <div class="fabrics-view" :class="{ rtl: isRtl }">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">
            {{
              t(
                'Manage manufacturing materials and textile specifications',
                'إدارة مواد التصنيع ومواصفات المنسوجات'
              )
            }}
          </p>
        </div>
          ...
          <table class="modern-table">
            ...
            <tbody>
              <tr v-for="fabric in fabrics" :key="fabric.id">
                ...
                <td>
                  <span class="type-badge">{{ fabric.type }}</span>
                </td>
                <td class="text-right">
                  <div class="action-btns">
                    <button
                      v-if="can('fabrics.edit')"
                      class="action-btn edit"
                      :title="t('Edit', 'تعديل')"
                      @click="openEditModal(fabric)"
                    >
                      <svg
                        width="16"
                        height="16"
                    ...
                    <button
                      v-if="can('fabrics.delete')"
                      class="action-btn del"
                      :title="t('Delete', 'حذف')"
                      @click="deleteFabric(fabric.id)"
                    >
                      <svg
                        width="16"
                        height="16"
              ...
              <tr v-if="fabrics.length === 0">
                <td colspan="3" class="empty-state">
                  {{ t('No fabrics found.', 'لم يتم العثور على أقمشة.') }}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div class="mt-4 flex flex-col gap-4 md:hidden">
        <div v-for="fabric in fabrics" :key="'mob-' + fabric.id" class="mobile-fabric-card">
          <div class="mfc-header">
            <h3 class="mfc-title">{{ fabric.name }}</h3>
            <span class="type-badge">{{ fabric.type }}</span>
          </div>

          <div class="mfc-actions mt-3 border-t border-slate-100 pt-3">
            <button v-if="can('fabrics.edit')" class="mfc-btn edit" @click="openEditModal(fabric)">
              <svg
                width="18"
                height="18"
                viewBox="0 0 24 24"
      ...
      <BaseModal
        :show="showModal"
        :title="
          editingFabric ? t('Edit Fabric', 'تعديل قماش') : t('Add New Fabric', 'إضافة قماش جديد')
        "
        :is-rtl="isRtl"
        @close="closeModal"
      >
        ...
        <template #footer>
          ...
          <BaseButton :loading="saving" @click="saveFabric">{{
            t('Save Fabric', 'حفظ القماش')
          }}</BaseButton>
        </template>
      </BaseModal>
      ...
      <ConfirmModal
        ...
        :message="
          t('Are you sure you want to delete this fabric?', 'هل أنت متأكد من حذف هذا القماش؟')
        "
