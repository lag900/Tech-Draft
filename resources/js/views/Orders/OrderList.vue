  ...
  <Layout>
    <div class="orders-view" :class="{ rtl: isRtl }">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">
            {{ t('Manage production pipeline and tech packs', 'إدارة خط الإنتاج والملفات الفنية') }}
          </p>
        </div>
          ...
          <div class="filter-flex">
            <div class="search-field">
              ...
              <input
                ...
                :placeholder="
                  t(
                    'Search by order code, brand or product...',
                    'بحث عن رقم الطلب، البراند أو المنتج...'
                  )
                "
              />
            </div>
      ...
      <div v-if="viewMode === 'table'">
        <!-- Table Card Wrapper -->
        <div class="table-card-wrapper mx-auto max-w-[1400px] overflow-visible">
          <!-- Desktop Table View -->
          <div v-if="!isMobile">
              ...
              <table class="w-full border-separate border-spacing-0">
                <thead>
                  <tr class="table-header-row border-b border-gray-100">
                    <th class="w-[120px] px-6 py-4 text-start align-middle">
                      {{ t('ID', 'رقم') }}
                    </th>
                    <th class="w-[240px] px-6 py-4 text-start align-middle">
                      {{ t('Product / Brand', 'المنتج / البراند') }}
                    </th>
                    <th
                      v-if="user?.role !== 'client'"
                      class="w-[200px] px-6 py-4 text-start align-middle"
                    >
                      {{ t('Client', 'العميل') }}
                    </th>
                    <th class="w-[160px] px-6 py-4 text-start align-middle">
                      {{ t('Category', 'التصنيف') }}
                    </th>
                    <th class="w-[180px] px-6 py-4 text-start align-middle">
                      {{ t('Material', 'الخامة') }}
                    </th>
                    <th class="w-[180px] px-6 py-4 text-center align-middle">
                      {{ t('Status', 'الحالة') }}
                    </th>
                    <th class="w-[180px] px-6 py-4 text-end align-middle">
                      {{ t('Actions', 'الإجراءات') }}
                    </th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                  ...
                  >
                    <!-- ID Column -->
                    <td class="px-6 py-5 text-start align-middle">
                    ...
                    <td class="px-6 py-5 text-start align-middle">
                      <div class="flex min-w-0 flex-col">
                        <span class="product-title transition-colors group-hover:text-blue-600">
                          {{ order.title || t('Untitled Design', 'تصميم بدون اسم') }}
                        </span>
                        <span class="product-meta mt-1">
                          {{ order.production_details?.item_type || '---' }} •
                          {{ order.production_details?.fit || '---' }}
                        </span>
                      </div>
                    </td>

                    <!-- Client Column -->
                    <td v-if="user?.role !== 'client'" class="px-6 py-5 text-start align-middle">
                    ...
                    <td class="px-6 py-5 text-start align-middle">
                      <div class="flex min-w-0 flex-col">
                        <span
                          class="max-w-[150px] truncate text-sm font-bold text-slate-700"
                          :title="order.fabric_details?.type || order.fabric_type"
                        >
                          {{ order.fabric_details?.type || order.fabric_type || '---' }}
                        </span>
                        <span
                          class="mt-1 max-w-[150px] truncate text-xs font-medium text-slate-400"
                          :title="order.notes"
                        >
                          {{ order.notes || '---' }}
                        </span>
                    ...
                    <!-- Status Column -->
                    <td class="px-6 py-5 text-center align-middle" @click.stop>
                      ...
                      <span
                        v-else
                        class="status-pill-v2 readonly mx-auto inline-flex"
                        :class="order.status"
                      >
                        {{ t(statusMap[order.status]?.en, statusMap[order.status]?.ar) }}
                      </span>
                    ...
                    <!-- Actions Column -->
                    <td class="px-6 py-5 text-end align-middle" @click.stop>
          ...
          <div v-if="filteredOrders.length === 0" class="border-t border-gray-50 py-20 text-center">
            ...
            <h3 class="mb-1 font-extrabold text-gray-900">
              {{ t('No orders discovered', 'لم يتم العثور على طلبات') }}
            </h3>
            <p class="text-sm text-gray-400">
              {{
                t(
                  'Try adjusting your search criteria or filters.',
                  'حاول تعديل معايير البحث أو خيارات التصفية.'
                )
              }}
            </p>
          </div>
              ...
              <template #item="{ element: order }">
                <div class="kanban-card" @click="$router.push('/orders/' + order.id)">
                  ...
                  <div v-if="order.status_history?.length" class="card-audit">
                    ...
                    <span
                      >{{ t('By', 'بواسطة') }} {{ order.status_history[0].changer.name }} •
                      {{ timeAgo(order.status_history[0].created_at) }}</span
                    >
                  </div>
                  ...
                  <div class="card-footer">
                    ...
                    <button
                      v-if="can('orders.approve') && nextStatusMap[status]"
        ...
        <Transition name="fade">
          <div v-if="showDeleteModal" class="modal-overlay" @click="showDeleteModal = false">
            <div class="delete-modal-card" @click.stop>
              ...
              <p>
                {{ t('Are you sure you want to delete order', 'هل أنت متأكد من حذف الطلب') }}
                <span class="font-bold">#{{ orderToDelete?.order_code }}</span
...
<script setup>
  ...
  const dynamicStats = computed(() => {
    return {
      ...
      technical_ready: {
        en: 'Tech Ready',
        ar: 'جاهز فنياً',
        count: statsData.value.technical_ready || 0,
      },
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      currentPage.value =
        sourceInfo.current_page || sourceInfo.currentPage || currentPage.value || 1;
      lastPage.value = sourceInfo.last_page || sourceInfo.lastPage || 1;
      totalOrders.value = sourceInfo.total ?? finalItems.length;

      console.log(
        `[OrderList] Mapping Success: ${finalItems.length} records, Page: ${currentPage.value}/${lastPage.value}, Total: ${totalOrders.value}`
      );
    } catch (err) {
  ...
  const updateOrderStatus = async (order, newStatus) => {
    ...
    try {
      ...
      showToast(
        t('Failed to update status. Reverting...', 'فشل تحديث الحالة. جاري التراجع...'),
        'error'
      );
      await fetchOrders();
      await fetchStats();
  ...
  const fabrics = computed(() =>
    [...new Set(orders.value.map((o) => o.fabric_details?.type))].filter(Boolean)
  );
  ...
  // We strictly watch it on 'orders' assignment so vuedraggable's local mutations don't trigger recursive rebuilds.
  ...
  const formatTime = (dateStr) => {
    if (!dateStr) return '';
    return new Date(dateStr).toLocaleDateString(isRtl.value ? 'ar-EG' : 'en-US', {
      month: 'short',
      day: 'numeric',
    });
  };
...
<style scoped>
  ...
  .stat-submitted .stat-indicator {
    background: #94a3b8;
  }
  .stat-pending .stat-indicator {
    background: #f59e0b;
  }
  .stat-technical_ready .stat-indicator {
    background: #3b82f6;
  }
  .stat-in_review .stat-indicator {
    background: #0ea5e9;
  }
  .stat-sampling .stat-indicator {
    background: #8b5cf6;
  }
  .stat-approved .stat-indicator {
    background: #10b981;
  }
  .stat-production .stat-indicator {
    background: #f97316;
  }
  .stat-completed .stat-indicator {
    background: #22c55e;
  }
  .stat-cancelled .stat-indicator {
    background: #ef4444;
  }
  ...
  .status-pill-v2.submitted {
    background: #f1f5f9;
    color: #475569;
    border-color: #cbd5e1;
  }
  .status-pill-v2.pending {
    background: #fffbeb;
    color: #92400e;
    border-color: #fcd34d;
  }
  .status-pill-v2.technical_ready {
    background: #eff6ff;
    color: #1e40af;
    border-color: #93c5fd;
  }
  .status-pill-v2.in_review {
    background: #f0f9ff;
    color: #075985;
    border-color: #7dd3fc;
  }
  .status-pill-v2.sampling {
    background: #f5f3ff;
    color: #5b21b6;
    border-color: #c4b5fd;
  }
  .status-pill-v2.approved {
    background: #f0fdf4;
    color: #166534;
    border-color: #86efac;
  }
  .status-pill-v2.production {
    background: #fff7ed;
    color: #9a3412;
    border-color: #fdba74;
  }
  .status-pill-v2.completed {
    background: #ecfdf5;
    color: #065f46;
    border-color: #6ee7b7;
  }
  .status-pill-v2.cancelled {
    background: #fef2f2;
    color: #991b1b;
    border-color: #fca5a5;
  }

  .status-pill-v2.readonly {
    cursor: default;
  }

  .status-cell-relative {
    position: relative;
  }
  ...
  .rtl .card-status-accent {
    left: auto;
    right: 0;
  }

  .order-mobile-card.submitted .card-status-accent,
  .order-mobile-card.pending .card-status-accent {
    background: #94a3b8;
  }
  .order-mobile-card.technical_ready .card-status-accent {
    background: #3b82f6;
  }
  .order-mobile-card.in_review .card-status-accent {
    background: #0ea5e9;
  }
  .order-mobile-card.sampling .card-status-accent {
    background: #8b5cf6;
  }
  .order-mobile-card.approved .card-status-accent {
    background: #10b981;
  }
  .order-mobile-card.production .card-status-accent {
    background: #f59e0b;
  }
  .order-mobile-card.completed .card-status-accent {
    background: #16a34a;
  }
  .order-mobile-card.cancelled .card-status-accent {
    background: #ef4444;
  }
  ...
  .page-indicator-glow .current {
    color: #2563eb;
    font-weight: 900;
    font-size: 0.9375rem;
  }
  .page-indicator-glow .separator {
    color: #bfdbfe;
    font-weight: 400;
  }
  .page-indicator-glow .total {
    color: #64748b;
    font-weight: 700;
    font-size: 0.8125rem;
  }
  ...
  .column-dot.submitted,
  .column-dot.pending {
    background: #94a3b8;
  }
  .column-dot.technical_ready {
    background: #3b82f6;
  }
  .column-dot.in_review {
    background: #0ea5e9;
  }
  .column-dot.sampling {
    background: #8b5cf6;
