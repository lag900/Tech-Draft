    ...
    <header class="client-navbar no-print">
      <div class="nav-container">
        ...
        <div class="nav-actions">
          <button @click="toggleLang" class="lang-minimal-btn">
            {{ isRtl ? 'EN' : 'AR' }}
          </button>
          ...
          <div class="user-dropdown-wrapper" ref="dropdownRef">
            <button class="avatar-trigger" @click="isDropdownOpen = !isDropdownOpen">
              ...
              <svg :class="{ 'rotate': isDropdownOpen }" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m6 9 6 6 6-6"/></svg>
            </button>
            ...
            <Transition name="dropdown">
              <div v-if="isDropdownOpen" class="portal-dropdown">
                ...
                <router-link to="/orders/create" class="dropdown-item" @click="isDropdownOpen = false">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>
                  {{ t('Create Order', 'إنشاء طلب') }}
                </router-link>
                <div class="dropdown-divider"></div>
                <button @click="logout" class="dropdown-item logout-text">
...
.portal-logo { width: 32px; height: 32px; }
.logo-text { font-size: 1.125rem; font-weight: 800; letter-spacing: -0.02em; color: #1a3a5f; }
...
.bc-item { color: #94a3b8; text-decoration: none; }
.bc-item.active { color: #1e293b; font-weight: 700; }
.bc-sep { color: #e2e8f0; }
...
.portal-link:hover { color: #0ea5e9; }
.portal-link.router-link-active { color: #0ea5e9; }
...
.nav-icon-btn:hover { background: #f8fafc; color: #0ea5e9; border-color: #e2e8f0; }

.notifications-bell { color: #94a3b8; }
.bell-dot { position: absolute; top: 8px; right: 8px; width: 8px; height: 8px; background: #0ea5e9; border: 2px solid white; border-radius: 50%; }
...
.avatar-trigger img { width: 38px; height: 38px; border-radius: 12px; border: 2px solid white; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }
.avatar-trigger svg { color: #94a3b8; transition: transform 0.3s; }
.avatar-trigger svg.rotate { transform: rotate(180deg); }
...
.dropdown-user-name { font-weight: 800; font-size: 0.9375rem; color: #1a3a5f; }
.dropdown-user-role { font-size: 0.75rem; color: #94a3b8; font-weight: 700; margin-top: 2px; }
...
.logout-text { color: #ef4444; width: 100%; border: none; background: none; cursor: pointer; }
.logout-text:hover { background: #fef2f2; color: #ef4444; }
...
/* RTL */
.rtl .portal-dropdown { right: auto; left: 0; }
.rtl .portal-link.router-link-active::after { left: auto; right: 0; }
...
.client-bottom-item.router-link-active { color: #0ea5e9; }
