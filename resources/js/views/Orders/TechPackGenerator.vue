  ...
  <div class="tech-pack-viewer">
    ...
    <div class="viewer-toolbar no-print">
      <div class="toolbar-content">
        ...
        <div class="toolbar-right">
          <button class="btn btn-primary" @click="exportPDF" :disabled="downloading">
            <span v-if="downloading" class="loader-spinner"></span>
            <svg v-else width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>
            Export PDF
          </button>
        </div>
      </div>
    ...
    <div class="viewer-content">
      <div v-if="loading" class="flex items-center justify-center min-h-[50vh]">
        <div class="loader-spinner !border-[#4F46E5] !w-8 !h-8 border-4"></div>
      </div>
      
      <TechPackLayout v-else-if="unified.product.id" :orderData="unified" id="tech-pack-content" :class="{'is-exporting': downloading}" />
      ...
      <div v-else class="empty-state text-center py-20">
        ...
        <p class="text-gray-400 mt-2">Please ensure the order exists and you have permission to view it.</p>
      </div>
    </div>
  </div>
</template>
...
const fetchData = async () => {
  try {
    const { data } = await axios.get(`/api/orders/${route.params.id}/tech-pack`, { headers: authHeaders() });
    unified.value = data;
  } catch {
...
@media print {
  ...
  :deep(.saas-sidebar), :deep(.saas-nav) {
    display: none !important;
  }
}
